<template>
  ...
  <form @submit.prevent="addNewTodo">
    <div class="mb-5">
      ...
      <div class="m-2">
        <button type="button" class="btn btn-danger" @click="removeAllTodos">Remove All</button>
      </div>
      <div class="m-2">
        <button type="button" class="btn btn-success" @click="markAllDone">Mark All Done</button>
      </div>
    </div>
    <div>
      ...
      <div v-else>
        <ul class="list-group">
          <li
            ...
            :key="todo.id"
          >
            <h3 :class="{ mark: todo.done }" style="cursor: pointer" @click="toggleDone(todo)">
              {{ todo.content }}
            </h3>
            <button type="button" class="btn btn-warning" @click="removeTodo(index)">
              ✔ Done & Remove
            </button>
          </li>
        </ul>
